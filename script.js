...
function fehlerSpeichern(lessonKey, vocabItem) {
    ...
    if (!fehlerDaten[vocabKey]) {
        fehlerDaten[vocabKey] = { first: vocabItem.first, second: vocabItem.second, count: 0 };
        if (vocabItem.third) fehlerDaten[vocabKey].third = vocabItem.third;
    }
...
function sessionFehlerHinzufügen(vocabItem) {
    ...
    if (existing) {
        ...
        let entry = { first: vocabItem.first, second: vocabItem.second, count: 1 };
        if (vocabItem.third) entry.third = vocabItem.third;
        session_fehler.push(entry);
    }
}
...
buildTableovid_test_eins(ovid_test_eins)

function buildTableovid_test_eins(data) {
	document.getElementById('ovid_test_eins_Anzeigetext').innerHTML = ovid_test_eins_Anzeigetext
	var tableovid_test_eins = document.getElementById('tableovid_test_eins')
	...
	for (var i = 0; i < data.length; i++) {
		...
		tableovid_test_eins.innerHTML += row
	}
}
...
buildTableenglisch_12_2__29_04_26(englisch_12_2__29_04_26)
function buildTableenglisch_12_2__29_04_26(data) {
	document.getElementById('englisch_12_2__29_04_26_Anzeigetext').innerHTML = englisch_12_2__29_04_26_Anzeigetext
	var tableenglisch_12_2__29_04_26 = document.getElementById('tableenglisch_12_2__29_04_26')
	for (var i = 0; i < data.length; i++) {
		var row = `<tr class="row">
							<td class="first">${data[i].first}</td>
							<td>${data[i].second}</td>
						</tr>`
		tableenglisch_12_2__29_04_26.innerHTML += row
	}
}
...
urlMenu.onchange = function () {
	...
	if (userOption.value == "#lek_englisch_12_2__29_04_26") {
		hide_all_außerenglisch_12_2__29_04_26()
	}
...
selected_englisch_12_2__29_04_26 = document.getElementsByClassName('insgesamtenglisch_12_2__29_04_26')
...
function hide_all() {
	...
	selected_englisch_12_2__29_04_26[0].classList.add('hidden');
...
function all() {
	...
	selected_englisch_12_2__29_04_26[0].classList.remove('hidden');
...
function hide_all_außerovid_test_eins() {
	hide_all()
	selected_ovid_test_eins[0].classList.remove('hidden');
	if(vokabeltest_lektion_anzeigen == true) {
		selected_Vokabeltest_Lektion[0].classList.add('hidden');
...
function hide_all_außerenglisch_12_2__29_04_26() {
	hide_all()
	selected_englisch_12_2__29_04_26[0].classList.remove('hidden');
	if(vokabeltest_lektion_anzeigen == true) {
		selected_Vokabeltest_Lektion[0].classList.add('hidden');
...
function lernen() {
	...
	urlMenu.onchange = function lernen_viva_lektion_select_function() {
		...
		if (userOption.value == "lernen_originaltexte_select_ovid_test_eins") {
			ausgewählt_lektion = "ovid_test_eins"
			lernen_noch_länge = ovid_test_eins.length
			lektion_ausgewählt = true
		}
		...
		if (userOption.value == "lernen_originaltexte_select_englisch_12_2__29_04_26") {
			ausgewählt_lektion = "englisch_12_2__29_04_26"
			lernen_noch_länge = englisch_12_2__29_04_26.length
			lektion_ausgewählt = true
		}
	...
	urlMenu.onchange = function lernen_englisch_select_function() {
		...
		if (userOption.value == "lernen_englisch_select_englisch_12_2__29_04_26") {
			ausgewählt_lektion = "englisch_12_2__29_04_26"
			lernen_noch_länge = englisch_12_2__29_04_26.length
			lektion_ausgewählt = true
		}
...
document.getElementById('lernen_englisch_select_englisch_12_2__29_04_26').innerHTML = englisch_12_2__29_04_26_Anzeigetext + " - " + englisch_12_2__29_04_26.length + " W";
...
if (lektion_ausgewählt === true){
			...
			letztes_wort = "";
			session_fehler = [];
			session_vokabeln_abgefragt = false;
...
function nicht_geschaft_länger_1() {
	if (lösung_angezeigt == true) {
		gesamt_fehleranzahl ++
		let fehlerFirst = swap_array ? zweites_wort : erstes_wort;
		let fehlerSecond = swap_array ? erstes_wort : zweites_wort;
		fehlerSpeichern(ausgewählt_lektion, { first: fehlerFirst, second: fehlerSecond, third: drittes_wort });
		sessionFehlerHinzufügen({ first: fehlerFirst, second: fehlerSecond, third: drittes_wort });
...
function neue_vok_standard() {
	...
	if(ausgewählt_lektion == "englisch_12_2__29_04_26") {
		if(swap_array == true) {
			swapped_array = swapFirstSecond(englisch_12_2__29_04_26)
			random_item = swapped_array[Math.floor(Math.random() * swapped_array.length)];
		} else {
			random_item = englisch_12_2__29_04_26[Math.floor(Math.random() * englisch_12_2__29_04_26.length)];
		}
		lernen_lektion_anzeige = englisch_12_2__29_04_26_Anzeigetext
		länge_lek = "Gesamt: " + englisch_12_2__29_04_26.length + " Wörter"
		länge_lek_number = englisch_12_2__29_04_26.length
	}
	...
	if (geschafte_wörter.includes(erstes_wort) || geschafte_wörter.includes(zweites_wort) || erstes_wort === letztes_wort) {
		...
		if (lernen_modus == "standard") {
			setTimeout(() => {
				...
				if (!session_vokabeln_abgefragt) session_vokabeln_abgefragt = true;
			}, 300);
		} else {
			setTimeout(() => {
				document.getElementById("lernen_lat").innerHTML = erstes_wort
...
function feedbackAnzeigen() {
    ...
    if (session_fehler && session_fehler.length > 0) {
        ...
        let sorted = session_fehler.slice().sort((a, b) => b.count - a.count);
        let table = '<table class="lernen_feedback_fehler_tabelle"><tr><th>Vokabel</th><th>Übersetzung</th><th>Fehler</th></tr>';
        for (let f of sorted) {
            table += '<tr><td>' + f.first + '</td><td>' + f.second + '</td><td>' + f.count + 'x</td></tr>';
        }
        table += '</table>';
        liste.innerHTML = table;
    } else if (session_vokabeln_abgefragt) {
        stats.innerHTML = 'Perfekt! Keine Fehler in dieser Session.';
        liste.innerHTML = '';
    } else {
        stats.innerHTML = 'Session beendet. Es wurden keine Vokabeln abgefragt.';
        liste.innerHTML = '';
    }
}
...
function fehlerlisteLektionWechseln() {
    ...
    let table = '<table class="fehlerliste_tabelle"><tr><th>Vokabel</th><th>Übersetzung</th><th>Fehler</th></tr>';
    for (let f of fehler) {
        table += '<tr><td>' + f.first + '</td><td>' + f.second + '</td><td>' + f.count + 'x</td></tr>';
    }
    table += '</table>';
...
function fehlerlisteSetReset() {
    let lessonKey = document.getElementById('fehlerliste_lektion_select').value;
    if (lessonKey && confirm('Moechtest du wirklich alle Fehler dieses Sets loeschen?')) {
        localStorage.removeItem('fehler_' + lessonKey);
        fehlerlisteLektionWechseln();
        document.getElementById('fehlerliste_inhalt').innerHTML = '<p>Keine Fehler fuer diese Lektion.</p>';
    }
}
